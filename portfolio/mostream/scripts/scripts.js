...
const heroMoviesList = [];

heroMoviesList.push(new HeroMovie(
...
const heroContentList = document.querySelector(".hero__content-list");
const heroContentItems = [];
const indicatorsList = document.querySelector(".hero__content-indicator-list");
const indicatorsItems = [];
const heroBackground = document.querySelector(".hero__background");

for (let i = 0; i < heroMoviesList.length; i++) {
	const item = document.createElement('div');
	item.classList.add('hero__content-item');
	item.innerHTML = `<div class="hero__content-item-anim"><h3 class="hero__content-title h2-inter semibold">${heroMoviesList[i].title}</h3><p class="hero__content-description h6-inter medium secondary">${heroMoviesList[i].description}</p></div>`;
	item.style.transform = `translateX(calc(${-i} * 100%))`;
	heroContentList.append(item);

	const itemWrapper = item.firstElementChild;
	itemWrapper.style.opacity = '0';
	heroContentItems.push(itemWrapper);
...
let movieIndex = 0;
let timeout;
heroContentItems[0].style.opacity = '1';
heroBackground.style.backgroundImage = `url(${heroMoviesList[0].backgroundImage})`;
indicatorsItems[0].classList.add('hero__content-indicator-item-active');
...
function setMovieIndex(index) {
	...
	heroContentItems[movieIndex].style.animationName = 'hero-item-hide';
	indicatorsItems[movieIndex].classList.remove('hero__content-indicator-item-active');
	movieIndex = index;
	heroContentItems[movieIndex].style.animationName = 'hero-item-show';
	heroBackground.style.backgroundImage = `url(${heroMoviesList[movieIndex].backgroundImage})`;
	indicatorsItems[movieIndex].classList.add('hero__content-indicator-item-active');
...
function nextMovie() {
	setMovieIndex((movieIndex + 1) % heroMoviesList.length);
}
